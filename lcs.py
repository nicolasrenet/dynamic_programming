        ...
        for col in range(2,last_col+1):
            ...
            if active_nodes[rw][cl] and not template:
                ...
                if active_nodes[rw][cl]==1:
                    ...
                    output.append( '\\node[select]\t({}-{}) at ({},{}*.75)\t{{$\\mathbf{{{}}}$}};'.format(col, row, col, row, c.get(rw,cl)))
...
def sample_adn(l):
    ...
    l2 = l - random.randint(0,4)

    x = []
    y = []

    for i in range(l):
        x.append( proteins[ random.randint(0,3) ])
        
    for i in range(l2):
        y.append( proteins[ random.randint(0,3) ])
    return (x,y)
